...
class RomanovMProhodJarvisaFuncTests : public ppc::util::BaseRunFuncTests<InType, OutType, TestType> {
 public:
...
const auto kFunctionalTasksList = std::tuple_cat(
    ppc::util::AddFuncTask<RomanovMProhodJarvisaMPI, InType>(kFunctionalTests, PPC_SETTINGS_romanov_m_prohod_jarvisa),
    ppc::util::AddFuncTask<RomanovMProhodJarvisaSEQ, InType>(kFunctionalTests, PPC_SETTINGS_romanov_m_prohod_jarvisa));

inline const auto kFunctionalGtestValues = ppc::util::ExpandToValues(kFunctionalTasksList);

TEST_P(RomanovMProhodJarvisaFuncTests, FunctionalTests) {
  ExecuteTest(GetParam());
}

INSTANTIATE_TEST_SUITE_P(Functional, RomanovMProhodJarvisaFuncTests, kFunctionalGtestValues,
                         RomanovMProhodJarvisaFuncTests::PrintTestName);

TEST(RomanovMProhodJarvisaValidation, MpiValidationFailsForLessThan3Points) {
  InType points = {{0, 0}, {1, 1}};
  RomanovMProhodJarvisaMPI task(points);

...
const auto kTasksList = std::tuple_cat(
    ppc::util::AddFuncTask<RomanovMProhodJarvisaMPI, InType>(kJarvisTests, PPC_SETTINGS_romanov_m_prohod_jarvisa),
    ppc::util::AddFuncTask<RomanovMProhodJarvisaSEQ, InType>(kJarvisTests, PPC_SETTINGS_romanov_m_prohod_jarvisa));
...
TEST(RomanovJarvisValidation, SeqFailsForSinglePoint) {
  InType points = {{0, 0}};
  RomanovMProhodJarvisaSEQ task(points);
  EXPECT_FALSE(task.Validation());
}
